...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate PerspectiveSpace diary images")
    parser.add_argument(
        "--root",
        default=None,
...
def discover_dates(root: Path, only_date: str | None) -> list[str]:
    ...
    for d in candidates:
        if (
            (diary_dir / f"{d}.npy").exists()
            and (sent_dir / d / "window_vectors.npy").exists()
        ):
            valid.append(d)
...
def build_style_context(root: Path, dates: list[str]) -> dict[str, np.ndarray]:
    cache_path = cache_dir(root) / f"style_context_{style_context_signature(root, dates)}.npz"
    if cache_path.exists():
        data = np.load(cache_path)
        return {"mean": data["mean"], "basis": data["basis"], "scale": data["scale"]}

    vecs = np.stack([np.load(root / "diary_vectors" / f"{d}.npy").reshape(-1) for d in dates])
...
def vec_style(diary_vec: np.ndarray, ctx: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    sat = float(np.clip(0.28 + 0.20 * abs(np.tanh(z1)) + 0.10 * abs(np.tanh(z2)), 0.2, 0.74))
...
def hsv_to_rgb255(h: float, s: float, v: float) -> tuple[float, float, float]:
    r, g, b = colorsys.hsv_to_rgb((h % 360.0) / 360.0, float(np.clip(s, 0, 1)), float(np.clip(v, 0, 1)))
    return (r, g, b)
...
def style_colors(
    ...
    accent_rgb = hsv_to_rgb255(style["hue"] + 14.0, min(style["sat"] + 0.14, 0.88), min(style["val"] + 0.06, 0.96))
    line_rgb = hsv_to_rgb255(style["hue"] + 36.0, min(style["sat"] + 0.20, 0.92), max(style["val"] - 0.48, 0.16))
...
def draw_background(
    ...
    for i in range(top_bands):
        ...
        c = hsv_to_rgb255(style["hue"] - 6.0, style["sat"] * (0.9 - 0.2 * t), min(style["val"] + 0.08 - 0.10 * t, 0.98))
        ax.add_patch(plt.Rectangle((left, y0), right - left, y1 - y0, color=c, ec="none", zorder=0))

    for i in range(floor_bands):
        y0 = horizon_y + (floor_h * i / floor_bands)
        y1 = horizon_y + (floor_h * (i + 1) / floor_bands)
        t = i / max(floor_bands - 1, 1)
        c = hsv_to_rgb255(style["hue"] + 8.0, min(style["sat"] + 0.10 + 0.06 * t, 0.88), max(style["val"] - 0.12 - 0.14 * t, 0.26))
        ax.add_patch(plt.Rectangle((left, y0), right - left, y1 - y0, color=c, ec="none", zorder=0))

    # Horizon emphasis.
    ax.plot([left, right], [horizon_y, horizon_y], color=line_rgb, lw=raster_s(2.0), alpha=0.65, zorder=3)
    ax.plot([left, right], [horizon_y + 6, horizon_y + 6], color=line_rgb, lw=raster_s(0.9), alpha=0.40, zorder=3)
    return line_rgb
...
def collect_perspective_lines(
    ...
    for i in range(row_count):
        ...
        lines.append(
            SvgLine(
                points=np.array([[left, y + jitter], [right, y + jitter]], dtype=np.float64),
                width=lw,
                opacity=alpha,
            )
        )
    ...
    for idx, ((vx, vy), w) in enumerate(zip(points, weights)):
        ...
        for x0 in bottom_x:
            lines.append(SvgLine(points=np.array([[x0, bottom], [vx, vy]], dtype=np.float64), width=lw, opacity=alpha))
        for x0 in top_x:
            lines.append(SvgLine(points=np.array([[x0, horizon_y], [vx, vy]], dtype=np.float64), width=lw * 0.95, opacity=alpha * 0.78))
        for y0 in left_y:
            lines.append(SvgLine(points=np.array([[left, y0], [vx, vy]], dtype=np.float64), width=lw * 0.92, opacity=alpha * 0.74))
        for y0 in right_y:
            lines.append(SvgLine(points=np.array([[right, y0], [vx, vy]], dtype=np.float64), width=lw * 0.92, opacity=alpha * 0.74))
    ...
    for x0 in xs:
        lines.append(SvgLine(points=np.array([[x0, bottom], [center_x, center_y]], dtype=np.float64), width=1.1, opacity=0.35, zorder=5))
...
def draw_perspective_grid(
    ...
    for line in collect_perspective_lines(points, weights, left, right, horizon_y, bottom, rng):
        ax.plot(
            line.points[:, 0],
            line.points[:, 1],
            color=line_rgb,
...
def draw_noise(ax: plt.Axes, left: float, right: float, top: float, bottom: float, seed_val: int) -> None:
...
def write_svg_foreground(
    ...
    points, weights = event_vanishing_points(event_vecs, diary_vec, left, right, top, bottom, rng)
    lines = collect_perspective_lines(points, weights, left, right, horizon_y, bottom, rng)
...
def render_one(
    ...
    points, weights = event_vanishing_points(event_vecs, diary_vec, left, right, top, bottom, rng)
    draw_perspective_grid(ax, points, weights, line_rgb, left, right, horizon_y, bottom, rng)
    draw_noise(ax, left, right, top, bottom, date_seed)

    # Double border frame.
    frame_color = (*line_rgb, 0.95)
    ax.add_patch(plt.Rectangle((OUTER_MARGIN, OUTER_MARGIN), CANVAS_W - 2 * OUTER_MARGIN, CANVAS_H - 2 * OUTER_MARGIN, fill=False, ec=frame_color, lw=raster_s(6.0), zorder=7))
    ax.add_patch(plt.Rectangle((INNER_MARGIN, INNER_MARGIN), CANVAS_W - 2 * INNER_MARGIN, CANVAS_H - 2 * INNER_MARGIN, fill=False, ec=frame_color, lw=raster_s(1.4), zorder=7))

    # Footer marks.
    ax.text(INNER_MARGIN + 6, OUTER_MARGIN - 10, f"events {event_vecs.shape[0]}", color=frame_color, fontsize=raster_s(8.5), ha="left", va="center")
    ax.text(CANVAS_W - INNER_MARGIN - 6, OUTER_MARGIN - 10, date, color=frame_color, fontsize=raster_s(8.5), ha="right", va="center")
...
def main() -> None:
    ...
    root = (script_dir.parent if args.root is None else script_dir / args.root).resolve()
    ...
    for i, date in enumerate(dates, start=1):
        diary_vec = np.load(root / "diary_vectors" / f"{date}.npy").reshape(-1).astype(np.float64)
        event_vecs = np.load(root / "diary_sentence_vectors" / date / "window_vectors.npy").astype(np.float64)
        saved: list[str] = []
        if run_out is not None:
